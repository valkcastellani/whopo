...
import com.valkcastellani.whopo.config.KafkaSseConsumer;
import com.valkcastellani.whopo.config.KafkaSseProducer;
...
class WhopoKafkaResourceIT {
    ...
    void producesMessages() throws Exception {
        restMockMvc.perform(post("/api/whopo-kafka/publish?message=value-produce")).andExpect(status().isOk());
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/whopo-kafka/register"))
        ...
        for (int i = 0; i < 100; i++) {
            ...
            if (content.contains("data:value-consume")) {
                restMockMvc.perform(get("/api/whopo-kafka/unregister"));
                return;
            }
        }
